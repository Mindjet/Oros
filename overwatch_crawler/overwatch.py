...
_driver_path = os.path.join(os.path.dirname(
    __file__), 'driver', 'chromedriver.exe')
...
class OverwatchCrawler(object):
    def __init__(self):
        ...
        # no browser window will be open
        option.headless = True
        self.driver = webdriver.Chrome(
            executable_path=_driver_path, chrome_options=option)
    ...
    def get_hero_avatars(self):
        ...
        hero_avatar_url = [item.get_attribute(
            'style') for item in hero_wrapper_list]
    ...
    def get_hero_details(self):
        ...
        output = []
        hero_list = driver.find_elements_by_class_name(
        ...
        for i, hero in enumerate(hero_list):
            ...
            item = {}
            large_avatar = hero.find_element_by_class_name(
                'portrait').get_attribute('src')
            ...
            try:
                hero_name = driver.find_element_by_class_name(
                    'hero-name').get_attribute('innerText')
                hero_type = driver.find_element_by_class_name(
                    'hero-detail-role-name').get_attribute('innerText')
                hero_description = driver.find_element_by_class_name(
                    'hero-detail-description').get_attribute('innerText')
                hero_bio = driver.find_element_by_css_selector(
                    '#story>.hero-detail-wrapper>p.hero-detail-title').get_attribute('innerText')

                base_of_operation = driver.find_element_by_css_selector(
                    '.base>.hero-bio-copy').get_attribute('innerText')
                base_of_operation = get_info_after_colon(base_of_operation)
                affiliation = driver.find_element_by_css_selector(
                    '.affiliation>.hero-bio-copy').get_attribute('innerText')
    ...
    def crawl_story(self):
        driver = self.driver
        story_segment = driver.find_elements_by_css_selector(
            '.hero-bio-backstory>p')
        story = ''
        ...
        for item in media:
            ...
            if media_type == 'image':
                target = driver.find_element_by_css_selector(
                    'div.lightbox-container>div.lightbox-body>a.media-item')
